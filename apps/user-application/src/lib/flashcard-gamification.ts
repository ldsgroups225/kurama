...
function getStreakMultiplier(streakDays: number): number {
  if (streakDays >= 30)
    return FLASHCARD_MULTIPLIERS.streak[30]
  if (streakDays >= 14)
    return FLASHCARD_MULTIPLIERS.streak[14]
  if (streakDays >= 7)
    return FLASHCARD_MULTIPLIERS.streak[7]
  if (streakDays >= 3)
    return FLASHCARD_MULTIPLIERS.streak[3]
  return 1
}
...
function getComboBonus(combo: number): number {
  if (combo >= 30)
    return FLASHCARD_BONUSES.combo_30
  if (combo >= 25)
    return FLASHCARD_BONUSES.combo_25
  if (combo >= 20)
    return FLASHCARD_BONUSES.combo_20
  if (combo >= 15)
    return FLASHCARD_BONUSES.combo_15
  if (combo >= 10)
    return FLASHCARD_BONUSES.combo_10
  if (combo >= 5)
    return FLASHCARD_BONUSES.combo_5
  return 0
}
...
export function getCardDifficulty(easeFactor: number): 'easy' | 'medium' | 'hard' {
  if (easeFactor >= 2800)
    return 'easy' // High ease = easy card
  if (easeFactor >= 2200)
    return 'medium' // Medium ease = medium card
...
export function getIntervalStage(repetitions: number, lastReviewedAt?: string): FlashcardCard['intervalStage'] {
  if (!lastReviewedAt || repetitions === 0)
    return 'first_review'
